...
def get_influx_data(date_from: datetime.datetime = None, date_to: datetime.datetime = None):
    try:
        ...
        result = client.query(f"""SELECT max("value") * 1000 AS "mean_value" FROM "homeassistant"."autogen"."kWh" WHERE {f"time > '{date_from.strftime('%Y-%m-%dT%H:%M:%SZ')}' AND" if date_from else ""} {f"time < '{date_to.strftime('%Y-%m-%dT%H:%M:%SZ')}' AND" if date_to else ""} "entity_id"='today_s_pv_generation' GROUP BY time(1d) FILL(0)""")
        client.close()
